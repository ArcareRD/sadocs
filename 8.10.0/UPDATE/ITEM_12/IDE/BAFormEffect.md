...
* <p id="fieldbreak4" style="color:blue;font-weight:bold">更新欄位</p>
    ...
    * `(2)影響筆數`
        * 用途說明
        * 規格說明
            * 當表單設定類型=RWD、APP, 本欄位除能, 預設: 單筆, 否則 致能
            * 單一選項: 單筆 / 多筆
            * 系統預設: 單筆
    * `(3)指定檔區`
        * 用途說明
        * 規格說明
            * 當表單設定類型=RWD、APP, 本欄位除能, 否則 致能
    ...
    * `(9)目的欄位`
        ...
        * 規格說明
            * 參照 [挑選表單元件通則][link_ruledialog7], 從駐留表單中, 進行元件挑選, 回傳: 表單元件
                * 當表單設計類型=RWD, 限定: 元件須存在 表單種類存在【1.一頁單筆(單筆)、6.過濾條件查詢(條件多筆)、7.指定條件執行(單改)】且 檔區=表頭 者
                * 當表單設計類型=APP, 限定: 元件須存在 表單種類存在【6.過濾條件查詢(條件多筆)、7.指定條件執行(單改)】且 檔區=表頭 者
