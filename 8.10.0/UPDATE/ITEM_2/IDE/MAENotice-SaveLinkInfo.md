...
* <p id="fieldbreak1" style="color:blue;font-weight:bold">基本</p>
    ...
    * `(7)給值內容`
        * 用途說明
        * 規格說明
            * 當`(5)給值類別`=推播來源欄位: [挑選邏表元件通則][link_ruledialog6], 從駐留表單中, 進行元件挑選, 回傳:表單元件 ; 若元件為 個資加密且已解密欄位 或 密碼欄位, 則該元件顯示紅色字體紅色字體
            * 當`(5)給值類別`=表單元件: [挑選表單元件通則][link_ruledialog7], 從 [推播通知-主旨內文][link_MAENotice_fieldbreak3]的`(2)檢視表`中, 進行檢視表元件挑選, 回傳:檢視表元件. 因查表來源暫不支援個資解密, 故不提供變色處理
