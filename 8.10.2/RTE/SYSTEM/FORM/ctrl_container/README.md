...
* 整個檔區 : (ex.按鍵加註/基本設定/重顯網格/資料行更新方式 = 全部記錄)
    * 重顯畫面					
        * 清除元件內的所有記錄				
        * 重新下載從第一筆記錄開始的一批資料
        * 依據原駐留位置往前抓取一批資料並顯示，例如 : 原駐留筆停在元件容器的第五筆，重顯畫面後，也需停留在第五筆。
    ...
    * 備註
        ...
        * 特殊狀況二 : 駐留筆前面有新增/刪除的資料 : 以駐留筆往下讀一批資料，光棒仍停在原駐留筆，但原駐留筆位置會依據前面被刪除或新增的資料而有變化。
...
[整個檔區重顯]:attachment/refreshalias3.png "整個檔區重顯"
