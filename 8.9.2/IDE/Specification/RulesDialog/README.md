...
* `(2)開窗鍵`
    * 用途說明
    * 規格說明
        * 單據於編輯狀態 或 單據於瀏覽狀態 且 條件ID有值 時, 致能
...
* `(2)開窗鍵`
    * 用途說明
    * 規格說明
        * 單據於編輯狀態 或 單據於瀏覽狀態 且 多語料號有值 時, 致能
...
* `(2)開窗鍵`
    * 用途說明
    * 規格說明
        * 單據於編輯狀態 或 單據於瀏覽狀態 且 資料表料號有值 時, 致能
        * 開窗[資料表][link_Physical]
        * 過濾: 依各單據需求, 指定過濾條件
        * 回傳: 資料表料號、資料表名稱
        * 當存在資料表料號 且 與回傳的資料表料號不相同時, 顯示詢問訊息盒【標題: 系統訊息 / 訊息內容: 異動資料表將清除相關欄位, 是否繼續? / 按鍵: 確認、取消】
            * 確認: 關閉訊息, 清除相關的欄位或記錄
            * 取消: 關閉訊息, 保留原內容
...
* `(2)開窗鍵`
    * 用途說明
    * 規格說明
        * 單據於編輯狀態 或 單據於瀏覽狀態 且 檢視表料號有值 時, 致能
        * 開窗[檢視表][link_Logical]
        * 過濾: 依各單據需求, 指定過濾條件
        * 回傳: 檢視表料號、檢視表名稱
        * 當存在檢視表料號 且 與回傳的檢視表料號不相同時, 顯示詢問訊息盒【標題: 系統訊息 / 訊息內容: 異動檢視表將清除相關欄位, 是否繼續? / 按鍵: 確認、取消】
            * 確認: 關閉訊息, 清除相關的欄位或記錄
            * 取消: 關閉訊息, 保留原內容
* `(3)參數鍵`
    * 用途說明
    * 規格說明
        * 單據於編輯狀態 或 單據於瀏覽狀態 且 參數ID有值 時, 致能
        * 當檢視表料號為空, 顯示訊息盒【標題: 系統訊息 / 訊息內容: 請先設定 檢視表 / 按鍵: 確認】
            * 確認: 關閉訊息
...
* `(2)設定鍵`
    ...
    * 規格說明
        ...
        * 過濾: 依各單據需求, 指定過濾條件
        * 回傳: 表單料號、表單名稱
        * 當存在表單料號 且 與回傳的表單料號不相同時, 顯示詢問訊息盒【標題: 系統訊息 / 訊息內容: 異動表單將清除相關欄位, 是否繼續? / 按鍵: 確認、取消】
            * 確認: 關閉訊息, 清除相關的欄位或記錄
            * 取消: 關閉訊息, 保留原內容
...
* `(2)開窗鍵`
    * 用途說明
    * 規格說明
        * 單據於編輯狀態 或 單據於瀏覽狀態 且 全域變數料號有值 時, 致能
...
* 其它說明
    * 編輯存回, 當記錄刪除時, 須連帶刪除該條件式、條件訊息的儲存內容
...
* `(2)開窗鍵`
    * 用途說明
    * 規格說明
        * 單據於編輯狀態 或 單據於瀏覽狀態 且 錯誤訊息有值 時, 致能
        * 開窗[按鍵執行條件訊息][link_BAXCondition]
        * 回傳: 錯誤訊息名稱

## <p id="ruledialog14">挑選開放按鍵參數通則</p>
![pic][image_ruleDialog2]
* `(1)名稱欄位`
    * 用途說明
    * 規格說明
...
* `(2)開窗鍵`
    ...
    * 規格說明
        * 編輯狀態 時, 致能
        * 開窗[快顯選單][link_Quick]
        * 過濾: 指定的開放按鍵料號 及 依各單據需求, 指定其它過濾條件
        * 回傳: 參數名稱
...
<!-- 超連結 -->
[link_Condoper]: "條件式"
[link_Multilingual]: "多語詞庫"
[link_Quick]: "快顯選單"
[link_Physical]: "資料表"
[link_Logical]: "檢視表"
[link_Parameter]: "傳遞參數"
[link_GlobalVariable]: "全域變數"
[link_BAXCondition]: "按鍵執行條件訊息"
